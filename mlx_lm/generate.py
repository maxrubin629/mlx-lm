...
def _encoder_decoder_generate_step(
    prompt: mx.array,
    model: nn.Module,
    *,
    max_tokens: int,
    ...
    with mx.stream(generation_stream):
        ...
        prompt_progress_callback(0, total_prompt_tokens)
...
def generate_step(
    ...
    *,
    ...
    if getattr(model, "is_encoder_decoder", False):
        yield from _encoder_decoder_generate_step(
            prompt,
            model,
...
def speculative_generate_step(
    ...
    *,
    ...
    """
    if getattr(model, "is_encoder_decoder", False) or getattr(
        draft_model,
        "is_encoder_decoder",
        False,
    ):
